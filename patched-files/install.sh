...
echo "Systemstorage"
echo "-------------"
echo "Skipping import system storage backup because THATS WHY !"
...
/usr/local/bin/php -f ../tools/apply.php ${ENVIRONMENT} ../Configuration/settings.csv || { echo "Error while applying settings" ; exit 1; }
echo
...
/usr/local/bin/php -f ../tools/n98-magerun.phar sys:setup:run || { echo "Error while triggering the update scripts using n98-magerun" ; exit 1; }
